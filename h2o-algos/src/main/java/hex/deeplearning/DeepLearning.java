...
//        if (!mp._quiet_mode) Log.info("Initial model:\n" + model.model_info());
        ...
        for(;;) {
          model.iterations++;
          model.set_model_info(new DeepLearningTask2(_job._key, train, model.model_info(), rowFraction(train, mp, model), model.iterations).doAll(Key.make(H2O.SELF)).model_info());//replicated data + single node mode
